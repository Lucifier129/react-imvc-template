...
export default class Home extends Controller {
	...
	handleCheckAll = event => {
		...
		let form2 = _.mapValues(
			state.form2,
			(item, key) => ({...item, isWarn: !item.isValid})
		)
		UPDATE_STATE({ form2 })
	}
	handleClearWarning = event => {
		...
		let form2 = _.mapValues(
			state.form2,
			(item, key) => ({...item, isWarn: false})
		)

		UPDATE_STATE({ form2 })

		let postForm = _.mapValues(
			state.form2,
			item => item.value
		)
